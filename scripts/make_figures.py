...
def fig2_site_map(outdir):
    ...
    ax.set_title('Fig. 2 — Case Study Sites, Korea\n(Placeholder — replace with CartoPy/QGIS map)')
    ax.grid(True, alpha=0.3)
    ax.plot(126.898, 35.754, 'o', color='#f28e2b', ms=12, zorder=5, label='Case 1: Gimje (전북)')
    ax.plot(126.540, 35.070, 's', color='#4e79a7', ms=12, zorder=5, label='Case 2: Hampyeong (전남)')
...
def fig3_scenario_comparison(scenarios, outdir):
    ...
    fig.suptitle('Fig. 3 — AquaCrop-OSPy Irrigation Strategy Comparison', fontsize=12, y=1.01)
...
def fig4_pareto_front(pareto_rows, scenario_rows, outdir):
    ...
    ax.set_title('Fig. 4 — Pareto Front: Yield vs Irrigation\nCase 1 Gimje 2019 (NSGA-2, 30 generations)', fontsize=11)
...
def fig5_ndvi_timeseries(outdir):
    ...
    ax.set_ylim(-0.1, 1.0)
    ax.set_title('Fig. 5 — NDVI Seasonality at Confirmed Sites (2022)\nPaddy rice phenological signature: transplanting dip → maturation peak',
                 fontsize=10)
